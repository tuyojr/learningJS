...
/*
{{ code is the same as watchList above }}
The variable watchList holds an array of objects with information on several 
movies. Use reduce to find the average IMDB rating of the movies directed 
by Christopher Nolan. Recall from prior challenges how to filter data and 
map over it to pull what you need. You may need to create other variables, 
and return the average rating from getRating function. Note that the rating 
values are saved as strings in the object and need to be converted into numbers 
...
function getRating(watchList3) {
    ...
    let averageRating = watchList3

        // First, using filter method, lets look for movies directed by Christopher Nolan
        .filter(watchList3 => watchList3.Director === "Christopher Nolan")

        // Next, use map method, lets collect and convert the ratings to a number from string
        // using the number constructor
        .map(watchList3 => Number(watchList3.imdbRating))

        // Next, use the reduce method to sum all the ratings together and divide it by the number of 
        // Christopher Nolan movies
        .reduce((ratingsSum, ratings) => ratingsSum + ratings) / 
        watchList3.filter(watchList3 => watchList3.Director === "Christopher Nolan").length

    // Lastly, return the variable
...
/*
This is another way to solve the above.
...
function nonMutatingSort(arr) {
    ...
    to the one being sorted (remember that slice and concat return a new 
...
the hyphenated version for the URL. You can use any of the methods covered 
in this section, and don't use replace. Here are the requirements:
...
returns an array of all objects that have matching name and value pairs (second 
    argument). Each name and value pair of the source object has to be present 
...
function convertHTML(str) {
    ...
    // var charHTML = str.split("");

    // for(var i = 0; i < charHTML.length; i++){
    //   switch (charHTML[i]) {
    //     case "&":
    //      charHTML[i] = "&amp;";
    //      break;
    ...
    //    }
    // }
    // return charHTML = charHTML.join('')

    // Create an object to assign all the required all the HTML characters
    let charHTML = {
        "&": "&amp;",
        "<": "&lt;",
        ">": "&gt;",
    ...
    // use the .replace method to change each character to HTML characters
    str = str.replace(/&/g, charHTML["&"]);
    str = str.replace(/</g, charHTML["<"]);
    str = str.replace(/>/g, charHTML[">"]);
    str = str.replace(/"/g, charHTML["\""]);
    str = str.replace(/'/g, charHTML["'"]);
...
it is only divisible by 1 and 2. In contrast, 4 is not prime 
...
function orbitalPeriod(arr) {
    ...
    To make the code easier to edit and read, each part of the equation 
    is written separately.
    Create newArr to store the orbPeriod's.
    a is 2 times pi. The part that is a constant is on the global scope 
    while the rest is part of a function.
    Create a function, gerOrbPeriod() that will do the required work 
    for any amount of objects.
